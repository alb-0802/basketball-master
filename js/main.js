...
function preload() {
    ...
    game.load.image('hoop', 'assets/images/hoop.png');
		...
		game.load.image('lose0', 'assets/images/win3.png');
		...
		game.load.image('lose4', 'assets/images/lose4.jpeg');

        // Audio
		game.load.audio('score', 'assets/audio/kobe.mp3');
...
function create() {
  ...
  game.physics.p2.gravity.y = 0;
	...
	instructions.innerHTML = "Anleitung: Ziehe den Ball schnell zum Korb!";
...
function update() {
	...
	if (ball && ball.body.velocity.y > 0 && ball.body.y > 188 && !ball.isBelowHoop) {
		...
		if (ball.body.x > 151 && ball.body.x < 249) {
			emojiName = "win" + rand;
			current_score += 1;
			current_score_text.text = current_score + ",Weiter so!";
			score_sound.play();
		} else {
			emojiName = "lose" + rand;
			fail.play();
			...
			// 	high_score_text.text = 'High Score\n' + high_score; >> der highscore wird eh nicht erzielt 
			}
			current_score = 0;
			current_score_text.text = '';
			current_best_text.text = 'Dein Highscore';
...
function click(pointer) { // festhalten
...
function release(pointer) { // loslassen
